...
> `0.2.x` 버전에서 sound task의 `end` (task 인터페이스) 이벤트가 발생되면 자동으로 `destroy`가 실행되도록 바뀌었습니다.  
> (`0.1.x` 버전에서는 `destroy`가 실행되지 않았습니다.)

- [sound 이벤트를 받을 수 없음 문제](https://github.com/vulcan9/taskml.doc/issues/2)
...
- [task 노드의 이벤트 누적 문제](https://github.com/vulcan9/taskml.doc/issues/1)
